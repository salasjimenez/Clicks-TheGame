...
export class GameEngine {
  ...
  private readonly eventSequence: EventId[] = ['doubleClick', 'clickRain', 'discount', 'criticalFever'];
  ...
  get derived(): DerivedStats {
    ...
    const offlineEfficiency = Math.min(
      0.8,
      0.35 + this.state.upgrades.offlineBattery * 0.05 + this.state.deepPrestige.upgrades.timeDilation * 0.03
    );
    ...
    const criticalChance = Math.min(
      0.65,
      0.05 + this.state.upgrades.luckyChip * 0.02 + this.state.deepPrestige.upgrades.criticalMatrix * 0.01 + criticalEvent
    );
    ...
    return {
      clickPower: baseClick * quantum * prestigeBonus * deepProductionMultiplier * eventClick * coinBonus * comboMultiplier,
      cps: (this.state.upgrades.autoClicker * quantum * prestigeBonus * deepProductionMultiplier * overclock + eventCps) * turboMultiplier * coinBonus,
      prestigeBonus,
  ...
  get deepPrestigeGain(): number {
    ...
    const progress = (this.state.prestigeLevel + this.state.prestigePoints) / this.deepPrestigeRequirement;
  ...
  playRoulette(choice: 'red' | 'black' | 'green'): { number: number; color: 'red' | 'black' | 'green'; reward: number } | null {
    ...
    if (number === 0 && choice === 'green') this.unlockAchievementById('ROULETTE_JACKPOT');
  ...
  playSlots(): { symbols: string[]; reward: number } | null {
    ...
    const reward = this.finishMinigame(rawReward);
    if (symbols.every((symbol) => symbol === '7️⃣')) this.unlockAchievementById('SLOT_JACKPOT');
  ...
  playGuess(choice: number): { answer: number; reward: number } | null {
    ...
    this.state.stats.guessWinStreak = won ? this.state.stats.guessWinStreak + 1 : 0;
    ...
    if (this.state.stats.guessWinStreak >= 10) this.unlockAchievementById('GUESS_MASTER');
  ...
  toggleSeason(): void {
    ...
    this.toast(this.state.season.enabled ? '✨ Temporada activada.' : '⏸️ Temporada pausada.', 'info');
    this.emit();
  }
  ...
  private checkBackgroundUnlocks(): void {
    if (this.state.manualClicks >= 1000 && !this.state.background.imageUnlockNotified) {
      this.state.background.imageUnlockNotified = true;
      this.toast('¡Desbloqueaste fondos de imagen!', 'success');
    }
    if (this.state.manualClicks >= 5000 && !this.state.background.videoUnlockNotified) {
  ...
  private finishMinigame(rawReward: number, winOverride: boolean | null = null): number {
    this.state.stats.minigamesPlayed += 1;
    this.state.daily.counters.minigames += 1;
    const reward = rawReward > 0 ? Math.max(1, Math.floor(rawReward * this.derived.minigameMultiplier)) : 0;
    const won = winOverride ?? reward > 0;
    if (reward > 0) this.awardCoins(reward, 'reward');
    if (won) {
      this.state.stats.minigamesWon += 1;
      this.toast(reward > 0 ? `🕹️ Ganaste ${this.format(reward)} monedas.` : '🕹️ Ronda completada.', 'success');
    } else if (reward > 0) {
  ...
  private ensureSeasonState(): void {
    ...
    this.state.season = { id: definition.id, enabled, points: 0, lastDailyBonusDate: '' };
  }
  ...
  private updateDailyStreak(): void {
    ...
    if (this.state.season.enabled && this.state.season.lastDailyBonusDate !== today) {
  ...
  private startEvent(): void {
    ...
    const durationMultiplier = this.state.season.enabled ? 1 + this.state.deepPrestige.upgrades.seasonAntenna * 0.05 : 1;
  ...
  private finishEvent(): void {
    if (this.state.activeEvent?.participated) {
      ...
      this.toast(this.state.season.enabled ? '✨ Evento completado: +1 punto de temporada.' : '✨ Evento completado.', 'success');
    }
    this.state.activeEvent = null;
  ...
  private isAchievementUnlocked(achievement: Achievement): boolean {
    ...
    if (id.startsWith('DEEP_LEVEL_')) return this.state.deepPrestige.level >= achievement.requirement;
    if (id.startsWith('DEEP_SHARDS_')) return this.state.deepPrestige.totalShardsEarned >= achievement.requirement;
    ...
    if (id.startsWith('NEON_PLAY_')) return this.state.stats.neonRushPlayed >= achievement.requirement;
    if (id.startsWith('NEON_WIN_')) return this.state.stats.neonRushWins >= achievement.requirement;
    if (id.startsWith('NEON_SCORE_')) return this.state.stats.highestNeonRushScore >= achievement.requirement;
    if (id.startsWith('SEASON_POINTS_')) return this.state.stats.seasonPoints >= achievement.requirement;
    if (id === 'MISSION_STREAK_7' || id === 'MISSION_STREAK_30') return this.state.daily.streak >= achievement.requirement;
    if (id === 'SECRET_7') return this.state.unlockedAchievements.length >= ACHIEVEMENTS.length - 1;
    ...
    if (achievement.category === 'deepPrestige') return this.state.deepPrestige.level >= achievement.requirement;
    ...
    if (achievement.category === 'seasons') return this.state.stats.seasonPoints >= achievement.requirement;
    ...
    if (id === 'SECRET_2') return this.state.stats.minigamesWon >= achievement.requirement;
    if (id === 'SECRET_3') return this.state.manualClicks >= achievement.requirement;
    if (id === 'SECRET_4') return this.state.prestigeLevel >= achievement.requirement;
    if (id === 'SECRET_5') return this.state.stats.eventsCompleted >= achievement.requirement;
    if (id === 'SECRET_6') return this.state.unlockedAchievements.length >= achievement.requirement;
    if (id === 'SECRET_8') return this.state.playSeconds >= achievement.requirement;
    return false;
  }
