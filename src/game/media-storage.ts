...
export function validateLocalMedia(
  kind: LocalMediaKind,
  file: File,
): string | null {
...
export async function saveLocalMedia(
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    transaction.onerror = () =>
      reject(transaction.error ?? new Error("No se pudo guardar el fondo."));
    transaction.onabort = () =>
      reject(transaction.error ?? new Error("No se pudo guardar el fondo."));
  });
  database.close();
...
export async function loadLocalMedia(
  ...
  const result = await new Promise<Blob | null>((resolve, reject) => {
    ...
    request.onerror = () =>
      reject(request.error ?? new Error("No se pudo leer el fondo."));
  });
  database.close();
  return result;
}
...
function blobToDataUrl(blob: Blob): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    reader.onerror = () =>
      reject(reader.error ?? new Error("No se pudo exportar el fondo."));
...
export async function exportStoredMedia(
  settings: BackgroundSettings,
): Promise<PortableBackgroundMedia | undefined> {
...
export async function importStoredMedia(
  media: PortableBackgroundMedia | undefined,
): Promise<void> {
  if (!media) return;
  if (
    ...
    if (IMAGE_TYPES.has(image.type) && image.size <= IMAGE_MAX_BYTES) {
      await saveLocalMedia(
        "image",
        new File([image], "background-image", { type: image.type }),
      );
    }
  }
  if (
    ...
    if (VIDEO_TYPES.has(video.type) && video.size <= VIDEO_MAX_BYTES) {
      await saveLocalMedia(
        "video",
        new File([video], "background-video", { type: video.type }),
      );
    }
  }
}
