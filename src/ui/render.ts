...
type AchievementFilter = 'all' | 'clicks' | 'production' | 'prestige' | 'arcade' | 'daily' | 'other';

const CLICK_PALETTES: Record<UiPalette, readonly (readonly [string, string, string])[]> = {
  arcade: [
...
export class GameUI {
  ...
  private readonly achievementPreview = byId<HTMLElement>('achievement-preview');
  ...
  private readonly backgroundSettings = byId<HTMLElement>('background-settings');
  private readonly backgroundImageInput = byId<HTMLInputElement>('background-image-input');
  private readonly backgroundVideoInput = byId<HTMLInputElement>('background-video-input');
  ...
  showToast(message: ToastMessage): void {
    ...
    if (message.tone === 'success') this.audio.playSuccess(this.engine.state.soundEnabled);
    if (message.text.includes('Logro:')) this.flashBody('fx-achievement');
    else if (message.text.includes('Núcleo reiniciado')) this.flashBody('fx-deep-prestige');
  ...
  private bindEvents(): void {
    ...
    document.addEventListener('keydown', (event) => {
      ...
      if (event.key === 'Escape' && !this.modalBackdrop.hidden) this.closeModal();
    });

    byId<HTMLButtonElement>('sound-toggle').addEventListener('click', () => this.engine.toggleSound());
    byId<HTMLButtonElement>('help-button').addEventListener('click', () => this.openHelp());
    byId<HTMLButtonElement>('achievements-button').addEventListener('click', () => this.openAchievements());
    ...
    byId<HTMLButtonElement>('import-button').addEventListener('click', () => this.importFile.click());
    byId<HTMLButtonElement>('reset-button').addEventListener('click', () => this.openResetConfirmation());
    byId<HTMLButtonElement>('prestige-button').addEventListener('click', () => this.openPrestigeConfirmation());
    byId<HTMLButtonElement>('deep-prestige-button').addEventListener('click', () => this.openDeepPrestigeConfirmation());
    byId<HTMLButtonElement>('season-toggle').addEventListener('click', () => this.engine.toggleSeason());
    byId<HTMLButtonElement>('modal-close').addEventListener('click', () => this.closeModal());
    ...
    document.querySelectorAll<HTMLButtonElement>('[data-minigame]').forEach((button) => {
      button.addEventListener('click', () => {
    ...
    this.missionList.addEventListener('click', (event) => {
      ...
      if (button?.dataset.missionId) this.engine.claimMission(button.dataset.missionId);
    });
  ...
  private render(state: GameState, derived: DerivedStats): void {
    byId<HTMLButtonElement>('sound-toggle').classList.toggle('is-muted', !state.soundEnabled);
    ...
    byId<HTMLElement>('prestige-level-mini').textContent = String(state.prestigeLevel);
    byId<HTMLElement>('prestige-level-hud').textContent = String(state.prestigeLevel);
    byId<HTMLElement>('total-clicks-hud').textContent = this.engine.format(state.manualClicks);
    byId<HTMLElement>('multiplier-hud').textContent = `x${(derived.prestigeBonus * derived.deepProductionMultiplier * derived.comboMultiplier).toFixed(2)}`;
    byId<HTMLElement>('quick-prestige-points').textContent = String(state.prestigePoints);
    byId<HTMLElement>('prestige-level').textContent = String(state.prestigeLevel);
    byId<HTMLElement>('prestige-points').textContent = String(state.prestigePoints);
    byId<HTMLElement>('prestige-bonus').textContent = `x${derived.prestigeBonus.toFixed(2)}`;
    byId<HTMLElement>('prestige-requirement').textContent = this.engine.prestigeGain > 0
      ? `+${this.engine.prestigeGain} puntos`
      : `${this.engine.format(this.engine.prestigeRequirement)} clics`;
    byId<HTMLButtonElement>('prestige-button').disabled = this.engine.prestigeGain < 1;
    byId<HTMLElement>('daily-streak').textContent = String(state.daily.streak);
    byId<HTMLElement>('achievement-count-mini').textContent = String(state.unlockedAchievements.length);
    byId<HTMLElement>('achievement-total-mini').textContent = String(ACHIEVEMENTS.length);
    byId<HTMLElement>('lifetime-coins').textContent = this.engine.format(state.lifetimeCoins);
    byId<HTMLElement>('manual-clicks').textContent = this.engine.format(state.manualClicks);
    byId<HTMLElement>('highest-coins').textContent = this.engine.format(state.stats.highestCoins);
    byId<HTMLElement>('minigames-won').textContent = `${state.stats.minigamesWon}/${state.stats.minigamesPlayed}`;
    byId<HTMLElement>('critical-clicks').textContent = this.engine.format(state.stats.criticalClicks);
    byId<HTMLElement>('highest-combo').textContent = this.engine.format(state.stats.highestCombo);
    byId<HTMLElement>('passive-coins').textContent = this.engine.format(state.stats.passiveCoins);
    byId<HTMLElement>('play-time').textContent = formatDuration(state.playSeconds);
    this.comboConsole.hidden = state.combo < 2;
    byId<HTMLElement>('combo-value').textContent = `x${derived.comboMultiplier.toFixed(2)}`;
    byId<HTMLElement>('combo-count').textContent = `${state.combo}/${derived.comboLimit} golpes`;
    byId<HTMLElement>('combo-progress').style.width = `${Math.min(100, (state.combo / derived.comboLimit) * 100)}%`;
  ...
  private renderDeepPrestige(state: GameState, derived: DerivedStats): void {
    byId<HTMLElement>('deep-prestige-level').textContent = String(state.deepPrestige.level);
    byId<HTMLElement>('deep-shards').textContent = String(state.deepPrestige.shards);
    byId<HTMLElement>('deep-bonus').textContent = `x${derived.deepProductionMultiplier.toFixed(2)}`;
    byId<HTMLElement>('deep-prestige-requirement').textContent = this.engine.deepPrestigeGain > 0
      ? `+${this.engine.deepPrestigeGain} fragmentos`
      : `Prestigio ${this.engine.deepPrestigeRequirement}`;
    byId<HTMLButtonElement>('deep-prestige-button').disabled = this.engine.deepPrestigeGain < 1;
    const season = currentSeason();
    byId<HTMLElement>('season-name').textContent = `${season.emoji} ${season.name}`;
    byId<HTMLElement>('season-points').textContent = String(state.season.points);
  ...
  private achievementMarkup(achievement: Achievement, unlocked: boolean): string {
  ...
  private performClick(x: number, y: number): void {
    ...
    requestAnimationFrame(() => {
      ...
      if (result.combo >= 10 && result.combo % 10 === 0) this.clickButton.classList.add('is-combo-flash');
  ...
  private renderCountUp(element: HTMLElement, value: number): void {
    ...
    const animate = (time: number): void => {
      ...
      if (progress < 1) this.coinAnimationFrame = requestAnimationFrame(animate);
  ...
  private maybeRotateClickPalette(): void {
    ...
    const now = performance.now();
    if (this.clicksSincePaletteChange < 4 || now - this.lastPaletteAt < 900) return;
    this.clicksSincePaletteChange = 0;
    this.lastPaletteAt = now;
    const palettes = CLICK_PALETTES[this.engine.state.background.palette];
    let next = this.clickPaletteIndex;
    while (next === this.clickPaletteIndex && palettes.length > 1) next = Math.floor(Math.random() * palettes.length);
  ...
  private renderBackgroundControls(state: GameState): void {
    ...
    this.backgroundSettings.querySelectorAll<HTMLButtonElement>('[data-ui-palette]').forEach((button) => {
    ...
    this.backgroundSettings.querySelectorAll<HTMLButtonElement>('[data-background-preset], [data-background-action="upload-image"]').forEach((button) => {
      button.disabled = !imageUnlocked;
    });
    this.backgroundSettings.querySelectorAll<HTMLButtonElement>('[data-background-action="upload-video"]').forEach((button) => {
      button.disabled = !videoUnlocked;
    });
    const pauseButton = this.backgroundSettings.querySelector<HTMLButtonElement>('[data-background-action="toggle-video"]');
    if (pauseButton) {
    ...
    this.backgroundSettings.querySelectorAll<HTMLButtonElement>('[data-background-preset]').forEach((button) => {
  ...
  private async syncBackgroundVisual(): Promise<void> {
    ...
    try {
      ...
      if (sequence === this.backgroundSyncSequence) this.showToast({ text: 'No se pudo aplicar el fondo local.', tone: 'warning' });
    }
  }
  ...
  private async handleBackgroundAction(event: Event): Promise<void> {
    const target = event.target;
    if (!(target instanceof Element)) return;
    const button = target.closest<HTMLButtonElement>('[data-background-action], [data-background-preset], [data-ui-palette]');
    if (!button) return;
    ...
    const preset = button.dataset.backgroundPreset as BackgroundPreset | undefined;
    if (preset) {
    ...
    if (action === 'upload-image') {
      if (this.engine.state.manualClicks >= 1000) this.backgroundImageInput.click();
      return;
    }
    if (action === 'upload-video') {
      if (this.engine.state.manualClicks >= 5000) this.backgroundVideoInput.click();
      return;
    }
  ...
  private prepareModal(): void {
    if (this.modalBackdrop.hidden) this.lastFocused = document.activeElement instanceof HTMLElement ? document.activeElement : null;
    this.cleanupFeature();
  ...
  private showModal(): void {
    ...
    requestAnimationFrame(() => this.modal.querySelector<HTMLElement>('button, input, [tabindex="0"]')?.focus());
  }
  ...
  private achievementMatchesFilter(achievement: Achievement, filter: AchievementFilter): boolean {
    if (filter === 'all') return true;
    if (filter === 'clicks') return achievement.category === 'clicks';
    if (filter === 'production') return ['cps', 'autoClickers', 'multiplier'].includes(achievement.category);
    if (filter === 'prestige') return achievement.category === 'prestige' || achievement.category === 'deepPrestige';
    if (filter === 'arcade') return achievement.category === 'minigames';
    if (filter === 'daily') return ['missions', 'events', 'seasons'].includes(achievement.category);
  ...
  private async handleModalAction(event: Event): Promise<void> {
    ...
    if (action.startsWith('achievement-filter-')) this.openAchievements(action.replace('achievement-filter-', '') as AchievementFilter);
    ...
    if (action.startsWith('roulette-')) {
      const choice = action.replace('roulette-', '') as 'red' | 'black' | 'green';
    ...
    if (action === 'guess-play') {
      const input = this.modalContent.querySelector<HTMLInputElement>('#guess-input');
