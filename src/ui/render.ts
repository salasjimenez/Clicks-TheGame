...
import type { DerivedStats, GameState, StoreItemId, ToastMessage } from '../types.js';
...
export class GameUI {
  ...
  private readonly terminalPreview = byId<HTMLElement>('achievement-terminal-preview');
  ...
  private readonly starGateButton = byId<HTMLButtonElement>('star-gate-button');
  private imageLabFile: File | null = null;
  private imageLabFormat: ImageOutputFormat = 'image/png';
  ...
  private bindEvents(): void {
    ...
    byId<HTMLButtonElement>('theme-toggle').addEventListener('click', () => this.engine.toggleTheme());
    ...
    byId<HTMLButtonElement>('export-button').addEventListener('click', () => exportState(this.engine.state));
    ...
    document.querySelectorAll<HTMLButtonElement>('[data-minigame]').forEach((button) => {
      button.addEventListener('click', () => {
        if (button.dataset.minigame === 'roulette') this.openRoulette();
        if (button.dataset.minigame === 'slots') this.openSlots();
        if (button.dataset.minigame === 'guess') this.openGuess();
      });
    });
  ...
  private render(state: GameState, derived: DerivedStats): void {
    ...
    byId<HTMLElement>('multiplier-hud').textContent = `x${(derived.prestigeBonus * derived.comboMultiplier).toFixed(2)}`;
    ...
    byId<HTMLElement>('terminal-achievement-count').textContent = String(state.unlockedAchievements.length);
    ...
    byId<HTMLElement>('prestige-requirement').textContent = `${this.engine.format(this.engine.prestigeRequirement)} clics`;
    const prestigeButton = byId<HTMLButtonElement>('prestige-button');
  ...
  private achievementMarkup(achievement: (typeof ACHIEVEMENTS)[number], unlocked: boolean): string {
  ...
  private async handleModalAction(event: Event): Promise<void> {
    ...
    if (action === 'open-star-gate') this.openStarGate();
    if (action === 'open-image-lab') this.openImageLab();
    if (action === 'verify-star') await this.verifyStarFromModal(button);
    if (action.startsWith('image-format-')) this.selectImageFormat(action.replace('image-format-', '') as 'png' | 'jpeg' | 'webp');
    if (action === 'convert-image') await this.convertImage();
  ...
  private syncEasterEgg(): void {
    const unlocked = localStorage.getItem(IMAGE_LAB_KEY) === 'unlocked';
    this.imageLabButton.hidden = !unlocked;
    byId<HTMLElement>('github-star-label').textContent = unlocked ? 'IMAGE LAB UNLOCKED' : 'STAR REPOSITORY';
    this.starGateButton.classList.toggle('is-unlocked', unlocked);
  }

  private async updateGitHubStars(): Promise<void> {
    try {
      ...
      byId<HTMLElement>('github-star-count').textContent = String(repository.stargazers_count ?? 0);
    } catch {
      byId<HTMLElement>('github-star-count').textContent = '★';
    }
  }

  private openStarGate(): void {
  ...
  private async verifyStarFromModal(button: HTMLButtonElement): Promise<void> {
  ...
  private async hasStarredRepository(username: string): Promise<boolean> {
    for (let page = 1; page <= 10; page += 1) {
      ...
      if (users.some((user) => user.login?.toLowerCase() === username.toLowerCase())) return true;
      if (users.length < 100) return false;
    }
    return false;
  }

  private openImageLab(): void {
    if (localStorage.getItem(IMAGE_LAB_KEY) !== 'unlocked') {
      this.openStarGate();
      return;
    }
  ...
  private async handleModalChange(event: Event): Promise<void> {
    const target = event.target;
    if (!(target instanceof HTMLInputElement) || target.id !== 'image-source') return;
    ...
    if (!(preview instanceof HTMLImageElement) || !shell) return;
    ...
    try {
      ...
      if (data) data.textContent = `${preview.naturalWidth} × ${preview.naturalHeight} · ${this.formatFileSize(file.size)}`;
      const convert = this.modalContent.querySelector<HTMLButtonElement>('[data-modal-action="convert-image"]');
  ...
  private selectImageFormat(format: 'png' | 'jpeg' | 'webp'): void {
    this.imageLabFormat = `image/${format}` as ImageOutputFormat;
    this.modalContent.querySelectorAll<HTMLButtonElement>('.format-button').forEach((button) => {
      button.classList.toggle('is-active', button.dataset.modalAction === `image-format-${format}`);
    });
  }

  private async convertImage(): Promise<void> {
  ...
  private setImageLabStatus(text: string, tone: string): void {
