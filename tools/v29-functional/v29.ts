...
import {
  ...
  V29_RELEASE,
  applySpecializationDerived,
  classifyBuySignal,
  estimateUpgradeBenefit,
  priceEfficiency,
...
export function installV29Derived(engine: GameEngine): void {
  const tagged = engine as GameEngine & { [DERIVED_MARK]?: boolean };
  if (tagged[DERIVED_MARK]) return;
  ...
  Object.defineProperty(engine, 'derived', {
    configurable: true,
    get(): DerivedStats {
      const base = getter.call(engine) as DerivedStats;
...
function focusables(container: ParentNode): HTMLElement[] {
...
export class V29Features {
  ...
  private weeklySignature = '';
  private specializationSignature = '';
  private storeSignalSignature = '';
  ...
  private injectWeeklyPanel(): void {
    const panel = document.getElementById('panel-missions');
    if (!panel || document.getElementById('v29-weekly-section')) return;
  ...
  private injectSpecializationPanel(): void {
    const panel = document.getElementById('panel-store');
    if (!panel || document.getElementById('v29-specialization')) return;
    const section = document.createElement('section');
  ...
  private injectPreferences(): void {
    const panel = document.getElementById('panel-stats');
    if (!panel || document.getElementById('v29-preferences')) return;
    const section = document.createElement('section');
    ...
    if (saveActions) panel.insertBefore(section, saveActions);
    else panel.appendChild(section);
    this.syncPreferenceControls();
  }

  private injectChangelog(): void {
    if (document.getElementById('v29-changelog-dialog')) return;
  ...
  private bindEvents(): void {
    document.addEventListener('click', (event) => {
      ...
      const action = target.closest<HTMLElement>('[data-v29-action]')?.dataset.v29Action;
      if (action === 'claim-weekly') {
        const id = target.closest<HTMLElement>('[data-v29-weekly-id]')?.dataset.v29WeeklyId;
        if (id) this.claimWeekly(id);
      }
      if (action === 'choose-path') {
        const path = target.closest<HTMLElement>('[data-v29-path]')?.dataset.v29Path as SpecializationPath | undefined;
        if (path) this.choosePath(path);
      }
      if (action === 'buy-node') {
        const id = target.closest<HTMLElement>('[data-v29-node]')?.dataset.v29Node as SpecializationUpgradeId | undefined;
        if (id) this.buyNode(id);
      }
      if (action === 'open-changelog') this.openChangelog(false);
    ...
    const dialog = document.getElementById('v29-changelog-dialog') as HTMLDialogElement | null;
  ...
  private handleKeyboard(event: KeyboardEvent): void {
    ...
    const dialog = document.getElementById('v29-changelog-dialog') as HTMLDialogElement | null;
    const regularModal = document.querySelector<HTMLElement>('.modal-backdrop:not([hidden]) .modal');
    const container: HTMLElement | null = dialog?.open ? dialog : regularModal;
    if (!container) return;
  ...
  private renderWeekly(state: GameState): void {
    ...
    if (signature === this.weeklySignature) return;
  ...
  private renderSpecialization(state: GameState): void {
    const feature = readV29State(state);
    const signature = JSON.stringify([feature.specialization, state.prestigePoints]);
    if (signature === this.specializationSignature) return;
  ...
  private choosePath(path: SpecializationPath): void {
    const feature = readV29State(this.engine.state);
    if (feature.specialization.path === path) return;
    if (feature.specialization.path) {
      const cost = 3;
      ...
      if (!window.confirm(`Cambiar a ${SPECIALIZATION_LABELS[path]} cuesta 3 puntos de prestigio y reinicia los nodos de la rama actual. ¿Continuar?`)) return;
      ...
      updateV29State((state) => {
        ...
        Object.keys(state.specialization.levels).forEach((id) => {
          state.specialization.levels[id as SpecializationUpgradeId] = 0;
        });
      }, this.engine.state);
  ...
  private buyNode(id: SpecializationUpgradeId): void {
    const feature = readV29State(this.engine.state);
    const node = SPECIALIZATION_NODES.find((item) => item.id === id);
    if (!node || feature.specialization.path !== node.path) return;
  ...
  private renderStoreSignals(state: GameState, derived: DerivedStats): void {
    ...
    if (signature === this.storeSignalSignature) return;
    this.storeSignalSignature = signature;
    const scores = data.map((item) => item.score);
    for (const item of data) {
      const button = document.querySelector<HTMLButtonElement>(`[data-store-item="${item.id}"]`);
      const card = button?.closest<HTMLElement>('.store-card');
      const info = card?.querySelector<HTMLElement>('.store-info');
      if (!button || !card || !info) continue;
      ...
      let badge = info.querySelector<HTMLElement>('.v29-buy-signal');
      if (!badge) {
  ...
  private syncPreferenceControls(): void {
    const preferences = readV29State(this.engine.state).preferences;
    const cap = document.getElementById('v29-offline-cap') as HTMLSelectElement | null;
    const particles = document.getElementById('v29-particles-enabled') as HTMLInputElement | null;
    if (cap && cap.value !== String(preferences.offlineCapHours)) cap.value = String(preferences.offlineCapHours);
    if (particles) particles.checked = preferences.particlesEnabled;
  }

  private maybeOpenChangelog(): void {
    const feature = readV29State(this.engine.state);
    if (feature.lastSeenVersion !== V29_RELEASE) this.openChangelog(true);
  }

  private openChangelog(automatic: boolean): void {
    const dialog = document.getElementById('v29-changelog-dialog') as HTMLDialogElement | null;
  ...
  private notify(text: string, tone: 'success' | 'info' | 'warning' | 'danger'): void {
