...
function selectDailyTemplates(date) {
...
export function createDailyState(date = todayKey()) {
    const missions = selectDailyTemplates(date).map((mission) => ({
        ...mission,
        progress: 0,
        claimed: false,
    }));
...
export function normalizeState(value) {
    ...
    const state = {
        ...
        totalClicks: previousVersion > 0 && previousVersion < 4
        ...
        activeEvent: source.activeEvent && typeof source.activeEvent === "object"
            ? source.activeEvent
            : null,
        ...
        season: {
            ...
            lastDailyBonusDate: sameSeason && typeof sourceSeason?.lastDailyBonusDate === "string"
    ...
    if (daily && daily.date === todayKey() && Array.isArray(daily.missions)) {
        state.daily = {
            ...
            lastCompletedDate: typeof daily.lastCompletedDate === "string"
    ...
    if (state.comboExpiresAt < Date.now())
        state.combo = 0;
    return state;
}
