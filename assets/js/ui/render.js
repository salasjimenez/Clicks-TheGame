...
export class GameUI {
    ...
    bindEvents() {
        ...
        byId("export-button").addEventListener("click", () => {
            void exportState(this.engine.state).catch(() => this.showToast({
    ...
    render(state, derived) {
        ...
        byId("multiplier-hud").textContent =
        ...
        byId("minigames-won").textContent =
        ...
        byId("combo-value").textContent =
            `x${derived.comboMultiplier.toFixed(2)}`;
        byId("combo-count").textContent =
            `${state.combo}/${derived.comboLimit} golpes`;
        byId("combo-progress").style.width =
    ...
    renderNextAchievement(state) {
        const clickAchievements = ACHIEVEMENTS.filter((achievement) => achievement.category === "clicks" && !achievement.secret).sort((first, second) => first.requirement - second.requirement);
        ...
        const previous = [...clickAchievements]
            .reverse()
    ...
    renderStore(state, derived) {
        ...
        const consumables = CONSUMABLES.map((item) => {
            const available = item.currency === "coins"
    ...
    renderAchievementPreview(state) {
        ...
        const locked = ACHIEVEMENTS.filter((achievement) => !state.unlockedAchievements.includes(achievement.id) &&
            !achievement.secret);
        const preview = [...unlocked.slice(-5).reverse(), ...locked].slice(0, 5);
        this.achievementPreview.innerHTML = preview
            .map((achievement) => this.achievementMarkup(achievement, state.unlockedAchievements.includes(achievement.id)))
            .join("");
    }
    ...
    renderBackgroundControls(state) {
        ...
        this.backgroundSettings
            .querySelectorAll("[data-ui-palette]")
            .forEach((button) => {
        ...
        this.backgroundSettings
            .querySelectorAll('[data-background-preset], [data-background-action="upload-image"]')
            .forEach((button) => {
            button.disabled = !imageUnlocked;
        });
        this.backgroundSettings
            .querySelectorAll('[data-background-action="upload-video"]')
            .forEach((button) => {
        ...
        this.backgroundSettings
            .querySelectorAll("[data-background-preset]")
            .forEach((button) => {
            button.classList.toggle("is-selected", state.background.mode === "preset" &&
    ...
    openPanel(name) {
        ...
        this.modal.classList.toggle("modal-wide", name === "store" ||
            name === "stats" ||
            name === "achievements" ||
            name === "prestige");
        this.showModal();
    ...
    showModal() {
        ...
        requestAnimationFrame(() => this.modal
            .querySelector('button, input, [tabindex="0"]')
            ?.focus());
    }
    closeModal() {
        this.cleanupFeature();
    ...
    openAchievements(filter = "all") {
        ...
        const cards = filtered
            .map((achievement) => this.achievementMarkup(achievement, this.engine.state.unlockedAchievements.includes(achievement.id)))
            .join("");
        const tabs = [
            ["all", "Todos"],
            ["clicks", "Clics"],
    ...
    achievementMatchesFilter(achievement, filter) {
        ...
        if (filter === "prestige")
            return (achievement.category === "prestige" ||
    ...
    async handleModalAction(event) {
        ...
        if (action.startsWith("roulette-")) {
            ...
            this.showMinigameResult(`${colorEmoji} ${result.number}`, result.reward > 0
        ...
        if (action === "slots-spin") {
            ...
            if (!result)
                return;
            this.showMinigameResult(result.symbols.join(" "), result.reward > 0
                ? `Ganaste ${this.engine.format(result.reward)} monedas`
                : "No hubo premio");
        }
        if (action === "guess-play") {
            ...
            if (!result)
                return;
            this.showMinigameResult(`🔢 ${result.answer}`, result.reward > 0
