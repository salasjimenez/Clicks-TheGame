...
export function neonRushMarkup() {
...
export function mountNeonRush(root, onFinish) {
    const field = root.querySelector('#neon-rush-field');
    const target = root.querySelector('#neon-rush-target');
    const scoreElement = root.querySelector('#neon-rush-score');
    const timeElement = root.querySelector('#neon-rush-time');
    ...
    const updateTimer = () => {
        ...
        if (timeElement)
            timeElement.textContent = remaining.toFixed(1);
        if (remaining <= 0) {
            finish();
            return;
        }
